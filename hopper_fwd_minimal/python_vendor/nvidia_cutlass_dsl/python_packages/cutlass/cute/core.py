...
class _Pointer(Pointer):
    ...
    def store(
        ...
        *,
        ...
        # LLVM doesn't support load/store narrow precision per element
        tmp_val = value.ir_value(loc=loc, ip=ip)  # type: ignore[union-attr]
        if self.dtype.width == 8:
...
def _op_wrapper(
    ...
    *,
    ...
    if isinstance(input, Tensor):
        res = op_fn(input.value, loc=loc, ip=ip)  # type: ignore[attr-defined]
...
def recast_ptr(
    ...
    dtype = cvt_type
    value_type = ptr.type.value_type if dtype is None else dtype  # type: ignore[attr-defined]
    swizzle = swizzle_.type.attribute if swizzle_ is not None else None
    res_ty = _cute_ir.PtrType.get(value_type, ptr.memspace, ptr.alignment, swizzle)  # type: ignore[attr-defined]
