...
def sm103_make_smem_layout_sfa(
    ...
    *,
    ...
    :rtype: cute.Layout
    """
    mma_tiler_mnk = cast(tuple[int, int, int], mma_tiler)
    mma_shape_mk = cast(
        tuple[tuple[int, int], int, int],
        tiled_mma.partition_shape_A((mma_tiler_mnk[0], mma_tiler_mnk[2])),
    )
    mma_op = cast(Any, tiled_mma.op)
    sf_atom = Sm103BlockScaledBasicChunk(sf_vec_size, mma_op.a_major_mode).layout
...
def sm103_make_smem_layout_sfb(
    ...
    *,
    ...
    mma_tiler_mnk = cast(tuple[int, int, int], mma_tiler)
    mma_op = cast(Any, tiled_mma.op)
    sf_atom = Sm103BlockScaledBasicChunk(sf_vec_size, mma_op.b_major_mode).layout
    k_divisor = 4 if sf_vec_size == 16 else 2
    mma_sfb_tiler = (mma_tiler_mnk[1], mma_tiler_mnk[2] // k_divisor)
