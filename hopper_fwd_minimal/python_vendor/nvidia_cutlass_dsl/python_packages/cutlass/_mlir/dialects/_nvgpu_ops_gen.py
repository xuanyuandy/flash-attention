...
class CvtFPExtOp(_ods_ir.OpView):
  OPERATION_NAME = "nvgpu.cvt_fpext"

  _ODS_REGIONS = (0, True)

  def __init__(self, out, in_, *, rnd=None, packed_kind=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    if rnd is not None: attributes["rnd"] = (rnd if (
        ...
        not _ods_ir.AttrBuilder.contains('NVGPU_RoundingModeAttr')) else
          _ods_ir.AttrBuilder.get('NVGPU_RoundingModeAttr')(rnd, context=_ods_context))
    if packed_kind is not None: attributes["packed_kind"] = (packed_kind if (
        isinstance(packed_kind, _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('SubBytesPackedKindAttr')) else
          _ods_ir.AttrBuilder.get('SubBytesPackedKindAttr')(packed_kind, context=_ods_context))
    results.append(out)
  ...
  @builtins.property
  def packed_kind(self):
    return self.operation.attributes["packed_kind"]

  @packed_kind.setter
  def packed_kind(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["packed_kind"] = value

  @builtins.property
  def out(self):
    return self.operation.results[0]

def cvt_fpext(out, in_, *, rnd=None, packed_kind=None, loc=None, ip=None) -> _ods_ir.Value:
  return CvtFPExtOp(out=out, in_=in_, rnd=rnd, packed_kind=packed_kind, loc=loc, ip=ip).result

@_ods_cext.register_operation(_Dialect)
class CvtFPTruncOp(_ods_ir.OpView):
  OPERATION_NAME = "nvgpu.cvt_fptrunc"
...
def cvt_fptrunc(out, in_, *, rnd=None, packed_kind=None, loc=None, ip=None) -> _ods_ir.Value:
  return CvtFPTruncOp(out=out, in_=in_, rnd=rnd, packed_kind=packed_kind, loc=loc, ip=ip).result
...
class FmaPackedF32x2Op(_ods_ir.OpView):
  OPERATION_NAME = "nvgpu.fma.packed.f32x2"

  _ODS_REGIONS = (0, True)

  def __init__(self, a, b, c, rnd, *, ftz=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
...
def fma_packed_f32x2(a, b, c, rnd, *, ftz=None, loc=None, ip=None) -> _ods_ir.Value:
  return FmaPackedF32x2Op(a=a, b=b, c=c, rnd=rnd, ftz=ftz, loc=loc, ip=ip).result
...
class MulPackedF32x2Op(_ods_ir.OpView):
  OPERATION_NAME = "nvgpu.mul.packed.f32x2"

  _ODS_REGIONS = (0, True)

  def __init__(self, a, b, *, rnd=None, ftz=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
...
def mul_packed_f32x2(a, b, *, rnd=None, ftz=None, loc=None, ip=None) -> _ods_ir.Value:
  return MulPackedF32x2Op(a=a, b=b, rnd=rnd, ftz=ftz, loc=loc, ip=ip).result
