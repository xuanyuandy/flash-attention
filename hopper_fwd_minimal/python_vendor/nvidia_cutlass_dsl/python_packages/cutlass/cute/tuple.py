...
def find_if(
    ...
    *,
    ...
    :type pred_fn: Callable[[XTuple, Union[int, Tuple[int, ...]]], bool]
    ...
    **Examples:**
    ...
    for i in range(rank(t)):
        ...
        res_pos = _find_if_impl(curr, i, loc=loc, ip=ip)  # type: ignore[call-arg]
        if res_pos is not None:
            return res_pos
    return None
...
@dsl_user_op
def find(
    t: XTuple, x: int, *, loc: Any = None, ip: Any = None
) -> Union[int, Tuple[int, ...], None]:
