...
def truncf_to_narrow(
    ...
    if res_elem_ty in (T.f6E3M2FN(), T.f6E2M3FN()):
        packed_kind_str = (
            "u6_unpack_u8_e3m2" if res_elem_ty == T.f6E3M2FN() else "u6_unpack_u8_e2m3"
        )
        packed_kind_attr = ir.Attribute.parse(
            f"#nvgpu.subbytes_packedkind<{packed_kind_str}>"
        )
        i8_ty = recast_type(res_ty, T.i8())
        unpacked_i8 = nvgpu.cvt_fptrunc(
            i8_ty, src, rnd=rnd, packed_kind=packed_kind_attr, loc=loc, ip=ip
        )
        i6_ty = recast_type(res_ty, T.i(6))
        i6_val = arith.trunci(i6_ty, unpacked_i8, loc=loc, ip=ip)
        return builtin.unrealized_conversion_cast([res_ty], [i6_val], loc=loc, ip=ip)
    return nvgpu.cvt_fptrunc(res_ty, src, rnd=rnd, loc=loc, ip=ip)


def extf_from_narrow(
    res_ty: ir.Type,
    src: ir.Value,
    loc: Optional[ir.Location],
    ip: Optional[ir.InsertionPoint],
...
class Vector(ArithValue):
    ...
    def __init__(
        ...
        *,
        ...
        self._mlir_type = dtype.mlir_type if dtype else self.type.element_type  # type: ignore[attr-defined]
    ...
    def dtype(self) -> "type":
        ...
        if self._dtype is None:
            raise NotImplementedError(f"dtype is not avaiable for {self._mlir_type}")
        return self._dtype
