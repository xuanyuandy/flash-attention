...
@deprecated(
    "cute.nvgpu.cpasync.LoadCacheMode is deprecated, use cufte.nvgpu.LoadCacheMode instead"
)
...
class CopyBulkTensorTileG2SNonExecTrait(TmaTrait):
...
class CopyBulkTensorTileG2SMulticastNonExecTrait(TmaTrait):
    def with_(
        self,
        *,
        loc: Optional[ir.Location] = None,
...
class CopyBulkTensorTileS2GTrait(Trait):  # type: ignore[no-redef]
    pass
...
class CopyBulkS2STrait(Trait):
    def unpack(
        ...
        **kwargs: Any,
        ...
        attr_str = f"#cute_nvgpu.atom_copy_field_bulks2s<{TMA_CTA_RANK_FIELD_NAME}>"  # type: ignore[name-defined]
        attr = ir.Attribute.parse(attr_str)
        val = _cute_nvgpu_ir.atom_set_value(
            val,
            attr,
            Int32(cta_rank).ir_value(loc=loc, ip=ip),  # type: ignore[name-defined]
            loc=loc,
            ip=ip,
        )
        return val
