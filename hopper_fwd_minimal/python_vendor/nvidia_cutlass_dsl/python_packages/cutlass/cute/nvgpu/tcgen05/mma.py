...
class MmaTF32Op(MmaOp):
    ...
    def _verify(self) -> None:
        ...
        if rank(self.shape_mnk) == 2:
            object.__setattr__(self, "shape_mnk", (*self.shape_mnk, instruction_k))  # type: ignore[misc]
        if self.shape_mnk[2] != instruction_k:  # type: ignore[index]
            raise OpError(
                self,
                f"expects the instruction extent in the K-mode to be {instruction_k}, "
                f"but got {self.shape_mnk[2]}",  # type: ignore[index]
            )
