...
from cutlass.cutlass_dsl import JitArgAdapterRegistry, CuTeDSL as _CuTeDSL, T
...
class _FakeTensor(Tensor):
    ...
    def mlir_type(self) -> ir.Type:
        ...
        elem_type = T.i8() if self._dtype.width == 1 else self._dtype.mlir_type  # type: ignore[name-defined]
    ...
    @property
    def leading_dim(self) -> Union[int, tuple[int, ...], None]:
        return core.leading_dim(self._shape, self._stride)  # type: ignore[arg-type]
...
def make_fake_compact_tensor(
    ...
    **Examples:**
    ...
    stride = tuple(  # type: ignore[assignment]
        SymInt(width=stride_width, divisibility=s.divisibility)  # type: ignore[arg-type, attr-defined]
