...
class MmaFP8Op(WarpMmaOp):
    ...
    def __post_init__(self) -> None:
        # fp8 warp MMA intrinsics (e.g., @llvm.nvvm.mma.m16n8k32.row.col.e4m3)
        # are not supported by libnvvm in CTK 12.9. libnvvm emits them as
        # external function declarations instead of PTX mma.sync instructions,
        # causing ptxas to fail.
        ...
        if not target_version(min_version="13.1"):
            ...
            raise OpError(
                self,
                f"FP8 warp-level MMA (SM89) is not supported with "
                f"CuTeDSL built with CUDA Toolkit "
                f"{CUDA_VERSION.major}.{CUDA_VERSION.minor}.",
                suggestion="Please upgrade to the CuTeDSL package built with "
        ...
        if self.acc_dtype not in [Float32]:
            raise OpError(
                self,
                "expects the 'acc_dtype' Op parameter to be Float32 for FP8 MMA "
                "(Float16 accumulator is not supported by the NVVM backend)",
            )
...
class MmaSM120BlockScaledOp(MmaOp):
    ...
    admissible_archs = ["sm_120a", "sm_121a", "sm_120f"]

    def __post_init__(self) -> None:
        # Verify arch
        arch = CuTeDSL._get_dsl().get_arch_enum()
        if not arch in [Arch.sm_120a, Arch.sm_121a, Arch.sm_120f]:
            raise OpError(
                self,
