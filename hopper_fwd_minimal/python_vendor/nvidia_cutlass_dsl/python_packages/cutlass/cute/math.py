    ...
    **kwargs: object,
    ...
    for arg in args:
        if not isinstance(arg, TensorSSA) and (
            not isinstance(arg, Numeric) or not type(arg).is_float  # type: ignore[misc]
        ):
            raise TypeError(
...
def copysign(
    ...
    *,
    ...
    """
    # NVVM fails to verify libdevice's copysign, mark this as not supported
    # return _math_op(math.copysign, fastmath, a, b, loc=loc, ip=ip)
    raise DSLRuntimeError("Copysign is not supported on CTK 12.9")
