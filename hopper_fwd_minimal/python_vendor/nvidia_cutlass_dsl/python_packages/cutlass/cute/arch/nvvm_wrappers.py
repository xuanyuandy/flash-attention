...
from typing import Any, Optional, Tuple, Type, Union, Callable, Literal
    ...
    *,
    ...
    if mlir_type == T.f32():
        ...
        return _warp_redux_sync_nvvm(
            value, kind, mask_and_clamp, loc=loc, ip=ip
        )
...
def atomic_fmax(
    ...
    *,
    ...
    return Float32(
        llvm.bitcast(T.f32(), old_intval.ir_value(loc=loc, ip=ip), loc=loc, ip=ip)  # type: ignore[union-attr]
    )
...
def load(
    ...
    *,
    ...
    if is_vector:
        ...
        return scalar_dtype(result)  # type: ignore[misc]
...
def prefetch(
    ...
    **kwargs: Any,
    ...
    """
    Prefetch data or TMA descriptor to cache via inline PTX.

    :param addr: LLVM pointer to prefetch.
    :param cache_level: Cache level string ("L1" or "L2"). Mutually exclusive with tensormap.
    ...
    if tensormap:
        ...
        level = cache_level if cache_level is not None else "L1"
        llvm.inline_asm(
            None,
            [ptr_as_i64],
