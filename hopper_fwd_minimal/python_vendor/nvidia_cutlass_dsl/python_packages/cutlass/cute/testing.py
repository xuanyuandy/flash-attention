...
def _maybe_recast_from_f4_f6(input: TensorSSA, src_dtype: Type[Numeric]) -> TensorSSA:
    ...
    if src_dtype.width == 4:
        ...
        i6_vec = builtin.unrealized_conversion_cast(
            [T.vector(n, T.i(6))], [input.maybe_downcast()]
        )
        res_vect = vector.bitcast(T.vector(n * 6 // 8, T.i8()), i6_vec)
        return TensorSSA(res_vect, recast_shape, Int8)
    return input
...
def _convert(
    ...
    # Step 2. maybe recast from f4/f6 tensor
...
def benchmark(
    ...
    *,
    ...
    if workspace_generator is None:
        ...
        workspace_generator = lambda: kernel_arguments  # type: ignore[assignment, return-value]

    workspaces = [workspace_generator() for _ in range(workspace_count)]  # type: ignore[misc]
    ...
    # =========================================================================
    if use_cuda_graphs:
        # Validate callable type for CUDA Graphs compatibility
        # Only JIT-compiled functions can be captured into CUDA Graphs
        supported_jit_types = (
            cutlass.base_dsl.jit_executor.JitCompiledFunction,
            cutlass.base_dsl.jit_executor.JitExecutor,
        )
        if not isinstance(callable, supported_jit_types):
