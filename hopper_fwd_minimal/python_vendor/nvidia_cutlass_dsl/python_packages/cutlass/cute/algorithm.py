...
def gemm(
    ...
    **kwargs: Any,
    ...
    a_vals = [t.value for t in a_list]  # type: ignore[attr-defined]
    b_vals = [t.value for t in b_list]  # type: ignore[attr-defined]
    return _cute_ir.gemm(value, d.value, a_vals, b_vals, c.value, loc=loc, ip=ip)  # type: ignore[attr-defined]
...
def basic_copy(
    ...
    *,
    ...
    if is_static(src.shape) and is_static(dst.shape):
        ...
        return _cute_ir.copy(simt_copy, [src.value], [dst.value], loc=loc, ip=ip)  # type: ignore[attr-defined]
    ...
    *,
    ...
    if (num_common_elements > 1) and (num_bits_per_copy % 8 == 0):
        ...
        return _cute_ir.copy(
            simt_copy_atom,
            [tiled_src.value],  # type: ignore[attr-defined]
            [tiled_dst.value],  # type: ignore[attr-defined]
            loc=loc,
            ip=ip,
        )
...
def copy(
    ...
    **Examples**:
    ...
    if is_static(src_primary.shape[1]) and unroll_factor is not None:  # type: ignore[index]
        ...
        value = atom._unpack(loc=loc, ip=ip, **kwargs)
        pred_value = pred.value if isinstance(pred, Tensor) else pred  # type: ignore[attr-defined]

        src_vals = [t.value for t in src_list]  # type: ignore[attr-defined]
        dst_vals = [t.value for t in dst_list]  # type: ignore[attr-defined]
...
def prefetch(
    ...
    *,
    ...
    return _cute_ir.prefetch(value, src.value, loc=loc, ip=ip)  # type: ignore[attr-defined]
