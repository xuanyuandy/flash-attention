...
class _Tensor(Tensor):
    ...
    **Examples:**
        ...
        *,
        ...
        if self._dtype is None:
            if is_int_tuple(self.iterator):
                ...
                self._dtype = self.iterator.value_type  # type: ignore[attr-defined]
        ...
        **Examples:**
        ...
        if has_underscore(crd):
            ...
            res = _cute_ir.get_iter(
                slice_(self, crd, loc=loc, ip=ip).value,  # type: ignore[attr-defined]
                loc=loc,
                ip=ip,
            )
    ...
    # Cheat to let `Type(_Tensor())` to return cute.Tensor
    @property  # type: ignore[misc, no-redef]
    def __class__(self) -> Type[Tensor]:  # type: ignore[override]
        return Tensor
    ...
    def memspace(self) -> AddressSpace:
        if isinstance(self.iterator, Pointer):
            return self.iterator.memspace  # type: ignore[attr-defined]
...
def domain_offset(
    ...
    *,
    ...
    if isinstance(tensor.iterator, Pointer):
        return make_tensor(
            tensor.iterator.__add__(offset, loc=loc, ip=ip),  # type: ignore[call-arg, operator]
            tensor.layout,
            loc=loc,
            ip=ip,
        )
...
def print_tensor(
    ...
    **Example output:**
    ...
    _cute_ir.print_view(tensor.value, verbose=verbose, is_signed=signed, loc=loc, ip=ip)  # type: ignore[union-attr]
...
class TensorSSA(cutlass_arith.ArithValue):
    ...
    def __init__(self, value: ir.Value, shape: Shape, dtype: Type[Numeric]) -> None:
    ...
    def _apply_op(
        ...
        *,
        ...
        if (
            ...
            lhs_val = lhs.maybe_downcast()  # type: ignore[attr-defined]
            rhs_val = rhs.maybe_downcast()  # type: ignore[attr-defined]
            ...
            res = TensorSSA(res_vect, lhs._shape, res_type)  # type: ignore[attr-defined]
    ...
    @dsl_user_op
    def broadcast_to(
        self, target_shape: Shape, *, loc: Any = None, ip: Any = None
    ) -> "TensorSSA":
        """
    ...
    def _flatten_shape_and_coord(
        self, crd: Any, *, loc: Any = None, ip: Any = None
    ) -> Any:
    ...
    def to(
        ...
        *,
        ...
        if src_dtype.is_float and dtype.is_float:
            if src_dtype == Float4E2M1FN and dtype in (Float16, Float32):  # type: ignore[name-defined]
                res_vect = cvt_f4e2m1_f16_intrinsic(
                    src, size(self.shape), loc=loc, ip=ip
                )
                if dtype == Float32:  # type: ignore[name-defined]
    ...
    @dsl_user_op
    def ir_value(self, *, loc: Any = None, ip: Any = None) -> Any:
        return self
