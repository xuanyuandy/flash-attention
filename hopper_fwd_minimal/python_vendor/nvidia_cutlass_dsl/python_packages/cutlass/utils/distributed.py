...
def atomicAdd(
    ...
    *,
    ...
    return cute.arch.atomic_add(  # type: ignore[return-value]
        ptr=dst_ptr.llvm_ptr,  # type: ignore[attr-defined]
        val=val,
        sem="relaxed",
        scope="sys",
        loc=loc,
...
def multimem_red_release_gpu_add1(
    ...
    *,
    ...
    llvm.inline_asm(
        None,
        [lock_ptr.toint().ir_value(loc=loc, ip=ip)],  # type: ignore[attr-defined]
...
def red_add1(
    ...
    *,
    ...
    """
    cute.arch.red(  # type: ignore[attr-defined]
        lock_ptr.llvm_ptr,  # type: ignore[attr-defined]
        Int32(1),
        op="add",
        dtype="s32",
        sem=order,
    ...
    *,
    ...
    while result != expected_val:
        result = cute.arch.atomic_cas(
            ptr=lock_ptr.llvm_ptr,  # type: ignore[attr-defined]
    ...
    *,
    ...
    while result < expected_val:
        result = cute.arch.load(
            lock_ptr.llvm_ptr,  # type: ignore[attr-defined]  # addr: Pointer to memory location
...
def multimem_ld_reduce_128bit_base(
    ...
    *,
    ...
    mc_ptr_int = mc_ptr.toint(loc=loc, ip=ip).ir_value(loc=loc, ip=ip)  # type: ignore[attr-defined]
